...
urls = [f"https://www.ncaa.com/scoreboard/baseball/d1/{d}/all-conf" for d in
    list(pd.Series(pd.date_range(start="04/10/2023",periods=7)).dt.strftime('%Y/%m/%d'))]
...
stats = pd.merge(stats,pd.read_csv('data/2023Conferences.csv').drop(columns='Unnamed: 0'),how='left',on='school_id')
